...
const matchFactory = (opts = {}) => (patterns, ordered, strict = false) => {
  ...
  for (const [key, value] of ordered) {
    for (let i = 0; i < patterns.length; i++) {
      ...
      const len = node.value.length; // [Optimazation]: once patterns[i] is fully matched,
      // we don't want to keep checking it
  ...
  for (let i = 0; i < pointers.length; i++) {
    ...
    if (pointers[i] >= patterns[i].value.length && (!matchNode || patterns[i].value.length > matchNode.value.length)) {
