...
class Path extends _core.default.Component {
  constructor(config, context) {
    ...
    this.params = [];
    this.regex = (0, _pathToRegexp.default)(config.path, this.params, config.options);
  }

  match(ctx) {
    const {
      consumeMatched = true
    } = this.config; // const match = this.trie.match(ctx.path)
  ...
  compose() {
    return _core.Match.accept(ctx => this.match(ctx)).use(this.config.subcomponents);
  }
