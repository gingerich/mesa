...
class Server {
  ...
  listen(...args) {
    ...
    const serviceMiddleware = async (ctx, next) => {
      const response = await this.service.call(ctx, msg => next().then(() => msg)); // const response = await this.service.call(msgFromRequest(ctx), function (msg) {
    ...
    const serviceMount = this.options.prefix ? (0, _koaMount.default)(this.options.prefix, serviceMiddleware) : serviceMiddleware;
